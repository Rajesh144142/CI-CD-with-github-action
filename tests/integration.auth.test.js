// filepath: d:\CI-CD-and-testing\tests\integration.auth.test.js
...
describe('Auth integration', () => {
  test('signup and login success', async () => {
    ...
    const signupRes = await request(app).post('/auth/signup').send({ username, password });
  ...
  test('login with wrong password returns 401', async () => {
    ...
    const loginRes = await request(app).post('/auth/login').send({ username, password: 'badpass' });
